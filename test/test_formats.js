...
const probe = require("../");
const str2arr = require("../dist/lib/common").str2arr;
...
describe("File formats", function () {
  ...
  describe("JPEG", function () {
    ...
    it("should not fail on bad JPEG", async function () {
      ...
      let buf = Buffer.from(
        "FFD8 FFC0 0004 00112233 FFD9".replace(/ /g, ""),
        "hex"
      );
    ...
    it("should skip padding", async function () {
      let buf = Buffer.from(
        "FFD8 FFFFFFFFC00011 08000F000F03012200021101031101 FFFFD9".replace(
          / /g,
          ""
        ),
        "hex"
    ...
    it("should skip unknown bytes between segments", async function () {
      ...
      let buf = Buffer.from(
        "FFD8 FFE00002 AAAAAAAA FFC00011 08000F000F03012200021101031101 FFD9".replace(
          / /g,
          ""
        ),
        "hex"
      ...
      buf = Buffer.from(
        "FFD8 AAAAAAAA FFE00002 FFC00011 08000F000F03012200021101031101 FFD9".replace(
          / /g,
          ""
        ),
        "hex"
    ...
    it("coverage - 0xE1 segment but not exif", async function () {
      let buf = Buffer.from(
        "FFD8 FFE10010AAAAAAAAAAAAAAAAAAAAAAAAAAAA FFC00011 08000F000F03012200021101031101 FFFFD9".replace(
          / /g,
          ""
        ),
        "hex"
  ...
  describe("AVIF", function () {
    ...
    it("minimal AVIF file + EXIF", async function () {
      ...
      let size = await probe(
        Readable.from([Buffer.concat([Buffer.from(buf), exif_data])])
      );
  ...
  describe("AVIF (sync)", function () {
    ...
    it("coverage - mime types", async function () {
      ...
      assert.strictEqual(probe.sync(buf).mime, "image/heic-sequence");
      ...
      assert.strictEqual(probe.sync(buf).mime, "image/avif");
      ...
      assert.strictEqual(probe.sync(buf).mime, "image/heif-sequence");
      ...
      assert.strictEqual(probe.sync(buf).mime, "image/heif");
  ...
  describe("SVG", function () {
    ...
    it("should extract width info from viewbox", async function () {
      let size = await probe(
        Readable.from([Buffer.from('<svg viewbox="0 0 800 600"></svg>')])
      );
    ...
    it("should return width/height units", async function () {
      let size = await probe(
        Readable.from([Buffer.from('<svg width="5in" height="4pt"></svg>')])
      );
    ...
    it("should not parse HTML as SVG", async function () {
      let buf = Buffer.from(
        '<html><body><svg width="5" height="4"></svg></body></html>'
      );
    ...
    it("should skip initial comments and directives", async function () {
      let buf = Buffer.from(
        '<?xml version="1.0"?><!-- comment --><svg width="5" height="4"></svg>'
      );

      assert.deepStrictEqual(await probe(Readable.from([buf])), {
        width: 5,
        height: 4,
        type: "svg",
        mime: "image/svg+xml",
    ...
    it("should allow SVG namespace", async function () {
      let buf = Buffer.from(
        '<aaa:svg xmlns:aaa="http://www.w3.org/2000/svg" width="5" height="4"></aaa:svg>'
      );
    ...
    it("should skip BOM", async function () {
      let size = await probe(
        Readable.from([Buffer.from('\ufeff  <svg width="5" height="4"></svg>')])
      );
    ...
    describe("coverage", function () {
      ...
      it("too much data before svg", async function () {
        await assert.rejects(
          async () =>
            probe(
              Readable.from([Buffer.from("<svg"), Buffer.alloc(70000, 0x20)])
            ),
          /unrecognized file format/
        );
      });
      ...
      it("single quotes (viewbox)", async function () {
        let size = await probe(
          Readable.from([Buffer.from("<svg width='1in' viewbox='0 0 100 50'>")])
        );
      ...
      it("height, no width", async function () {
        let size = await probe(
          Readable.from([
            Buffer.from('<svg height="1in" viewbox="0 0 100 50">'),
          ])
        );
      ...
      it("width is invalid, no height", async function () {
        await assert.rejects(
          async () =>
            probe(
              Readable.from([
                Buffer.from('<svg width="-1" viewbox="0 0 100 50">'),
              ])
            ),
          /unrecognized file format/
        );
      ...
      it("height is invalid, no width", async function () {
        await assert.rejects(
          async () =>
            probe(
              Readable.from([
                Buffer.from('<svg height="foobar" viewbox="0 0 100 50">'),
              ])
            ),
          /unrecognized file format/
        );
      ...
      it("width is invalid (non positive)", async function () {
        await assert.rejects(
          async () =>
            probe(Readable.from([Buffer.from('<svg width="0" height="5">')])),
          /unrecognized file format/
        );
      });
      ...
      it("width is invalid (Infinity)", async function () {
        await assert.rejects(
          async () =>
            probe(
              Readable.from([Buffer.from('<svg width="Infinity" height="5">')])
            ),
          /unrecognized file format/
        );
      });
      ...
      it("viewbox units are different", async function () {
        await assert.rejects(
          async () =>
            probe(
              Readable.from([
                Buffer.from('<svg width="5" viewbox="0 0 5px 3in">'),
              ])
            ),
          /unrecognized file format/
        );
      ...
      it("early termination", async function () {
        await assert.rejects(
          async () =>
            probe(Readable.from([Buffer.from('<svg width="5" height="5"')])),
          /unrecognized file format/
        );
      });
    });
  ...
  describe("SVG (sync)", function () {
    ...
    it("should skip initial comments and directives", async function () {
      ...
      assert.deepStrictEqual(probe.sync(buf), {
        width: 5,
        height: 4,
        type: "svg",
        mime: "image/svg+xml",
    ...
    it("should skip BOM", async function () {
      let size = probe.sync(
        Buffer.from('\ufeff  <svg width="5" height="4"></svg>')
      );
    ...
    describe("coverage", function () {
      it("wrong signature", function () {
        let size = probe.sync(
          Buffer.from('  <not-really-svg width="1" height="1">')
        );
      ...
      it("single quotes (viewbox)", function () {
        let size = probe.sync(
          Buffer.from("<svg width='1in' viewbox='0 0 100 50'>")
        );
  ...
  describe("TIFF", function () {
    ...
    it("bad TIFF IFD", async function () {
      ...
      //                     sig     off      count next
      let buf = Buffer.from(
        "49492a00 08000000 0000 00000000".replace(/ /g, ""),
        "hex"
      );
    ...
    it("bad TIFF IFD offset", async function () {
      ...
      //                     sig     off      count next
      let buf = Buffer.from(
        "49492a00 00000000 0000 00000000".replace(/ /g, ""),
        "hex"
      );
  ...
  describe("WEBP", function () {
    ...
    it("should skip VP8 header with bad code block", async function () {
      let buf = Buffer.from(
        str2arr("RIFF\x20\0\0\0WEBPVP8 \x0C\0\0\0....................")
      );
    ...
    it("should skip VP8L header with bad code block", async function () {
      let buf = Buffer.from(
        str2arr("RIFF\x20\0\0\0WEBPVP8L\x0C\0\0\0....................")
      );
