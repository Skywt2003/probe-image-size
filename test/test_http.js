...
describe("probeHttp", function () {
  ...
  it("should abort request ASAP", async function () {
    ...
    await assert.rejects(async () => probe(url), /unrecognized file format/);
  });
  ...
  it("should fail on 404", async function () {
    ...
    await assert.rejects(async () => probe(url), { statusCode: 404 });
  });
  ...
  it("should fail on status 201-299 codes", async function () {
    ...
    await assert.rejects(async () => probe(url), { statusCode: 201 });
  });

  it("should return error if url is not a string", async function () {
    // for coverage
    await assert.rejects(async () => probe(123), /URL must be a string/);
  });
  ...
  it("should return error if connection fails", async function () {
    ...
    await assert.rejects(async () => probe(url, { retries: 0 }), {
      code: "ECONNRESET",
    });
  });
...
describe("probeHttpWithAgent", function () {
  ...
  it("should fail on 404", async function () {
    ...
    await assert.rejects(async () => probe(url, { agent: httpAgent }), {
      statusCode: 404,
    });
  });
      ...
      for (let hostTarget in httpAgent.sockets) {
        ...
        if (hostTarget) {
          httpAgent.sockets[hostTarget].forEach(function (socket) {
            socket
              .listeners("end")
              .length.should.be.below(
                5,
                "too many listeners on the socket object's end event"
              );
          });
        }
      }
